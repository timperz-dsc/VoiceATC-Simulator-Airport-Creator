...
function App() {
  ...
  const [showConfirmDeleteFix, setShowConfirmDeleteFix] = useState(false);
  ...
  // Load FIXES from localStorage on component mount
  useEffect(() => {
    try {
      const savedFixes = localStorage.getItem('fixes');
      if (savedFixes && savedFixes !== '[]' && savedFixes !== 'null') {
        const parsedFixes = JSON.parse(savedFixes);
        if (Array.isArray(parsedFixes) && parsedFixes.length > 0) {
          setFixes(parsedFixes);
          console.log('Loaded FIXES from localStorage:', parsedFixes);
        } else {
          console.log('Parsed data is not a valid FIXES array, using empty array');
          setFixes([]);
        }
      } else {
        console.log('No valid saved FIXES found in localStorage');
        setFixes([]);
      }
    } catch (error) {
      console.error('Error loading FIXES from localStorage:', error);
      setFixes([]);
    }
  }, []);

  // Save FIXES to localStorage whenever fixes change
  useEffect(() => {
    // Skip saving during initial load
    if (fixes.length === 0) {
      console.log('Skipping save - FIXES array is empty (likely initial load)');
      return;
    }
    
    try {
      // Only save if there are actually FIXES
      if (fixes && fixes.length > 0) {
        localStorage.setItem('fixes', JSON.stringify(fixes));
        console.log('Saved FIXES to localStorage:', fixes);
      } else {
        // Remove the key if no FIXES
        localStorage.removeItem('fixes');
        console.log('Removed FIXES from localStorage (no FIXES)');
      }
    } catch (error) {
      console.error('Error saving FIXES to localStorage:', error);
    }
  }, [fixes]);
  ...
  }, [airports, routes, runways, configs, geoEntries, mrvaEntries, glidepathEntries, sectors, pronunciations, fixes]);
  ...
  const debugLocalStorage = () => {
    ...
    console.log('fixes:', localStorage.getItem('fixes'));
    ...
    console.log('Current state fixes:', fixes);
    console.log('========================');
  };
  ...
  // FIX data handling functions
  const handleFixDataChange = (field, value) => {
    setFixData(prev => ({ ...prev, [field]: value }));
  };

  const handleAddFix = () => {
    // Check if all required fields are filled
    const requiredFields = ['name', 'coordinate', 'visibilityFix1', 'visibilityFix2'];
    const emptyFields = requiredFields.filter(field => !fixData[field]);
    ...
    const newFix = {
      id: Math.random().toString(36).substr(2, 9),
      name: fixData.name,
      coordinate: fixData.coordinate,
      visibilityFix1: fixData.visibilityFix1,
      visibilityFix2: fixData.visibilityFix2,
      createdAt: new Date().toISOString(),
    };
  ...
  const handleDeleteFix = (id) => {
    setFixes(prev => prev.filter(fix => fix.id !== id));
  };

  const handleEditFix = (fix) => {
    setEditingFix(fix);
    setFixData({
      name: fix.name,
      coordinate: fix.coordinate,
      visibilityFix1: fix.visibilityFix1,
      visibilityFix2: fix.visibilityFix2,
    });
  };

  const handleUpdateFix = () => {
    if (!editingFix) return;

    const updatedFixes = fixes.map(fix => 
      fix.id === editingFix.id 
        ? { ...fix, ...fixData }
        : fix
    );
  ...
  const filteredFixes = fixes.filter(fix =>
    fix.name.toLowerCase().includes(fixSearchTerm.toLowerCase()) ||
    fix.coordinate.toLowerCase().includes(fixSearchTerm.toLowerCase()) ||
    fix.visibilityFix1.toLowerCase().includes(fixSearchTerm.toLowerCase()) ||
    fix.visibilityFix2.toLowerCase().includes(fixSearchTerm.toLowerCase())
  );
                                    ...
                                    <div style={styles.navFourthLevelDropdownContainer}>
                                      <div 
                                        ...
                                        onClick={(e) => {
                                          ...
                                          // Toggle the view
                                          if (currentAirportView && currentAirportView.type === 'fix' && currentAirportView.airport.id === airport.id) {
                                            setCurrentAirportView(null);
                                          } else {
                                            setCurrentAirportView({ type: 'fix', airport: airport });
                                            setCurrentView('airport-view');
                                          }
                                          // Close all dropdowns
                                          setShowAirportsDropdown(false);
                                      ...
                                      >
                                        <span>FIX</span>
                                        {currentAirportView && currentAirportView.type === 'fix' && currentAirportView.airport.id === airport.id && (
                                          <span className="material-icons" style={{color: '#10b981', fontSize: '18px'}}>check</span>
                                        )}
                                      </div>
                                      <div 
      ...
      <div style={{
        ...
        {currentAirportView && (
          <div style={{flex: '1', overflow: 'auto', padding: '24px'}}>
            ...
            {currentAirportView.type === 'fix' && (
              <div>
                <h2 style={styles.cardTitle}>Fix Management for {currentAirportView.airport.name}</h2>
                
                {/* Input Section */}
                <div style={styles.card}>
                  <h2 style={styles.cardTitle}>Add New Fix</h2>
                  <div style={styles.grid}>
                    <div style={styles.inputGroup}>
                      ...
                      <input
                        ...
                        placeholder="BABAP"
                        value={fixData.name}
                        onChange={(e) => handleFixDataChange('name', e.target.value)}
                        style={styles.input}
                      />
                    </div>
                    ...
                    <div style={styles.inputGroup}>
                      ...
                      <input
                        type="text"
                        placeholder="N054.07.23.243 E018.07.24.276"
                        value={fixData.coordinate}
                        onChange={(e) => handleFixDataChange('coordinate', e.target.value)}
                        style={styles.input}
                      />
                    </div>
                    ...
                    <div style={styles.inputGroup}>
                      ...
                      <input
                        type="text"
                        placeholder="1"
                        value={fixData.visibilityFix1}
                        onChange={(e) => handleFixDataChange('visibilityFix1', e.target.value)}
                        style={styles.input}
                      />
                    </div>
                    ...
                    <div style={styles.inputGroup}>
                      ...
                      <input
                        type="text"
                        placeholder="2"
                        value={fixData.visibilityFix2}
                        onChange={(e) => handleFixDataChange('visibilityFix2', e.target.value)}
                        style={styles.input}
                      />
                    </div>
                  </div>
                  ...
                  {editingFix ? (
                    <div style={{display: 'flex', gap: '12px', marginTop: '24px'}}>
                      <button onClick={handleUpdateFix} style={styles.button}>
                        <span className="material-icons">save</span>
                        Update Fix
                      </button>
                      <button onClick={handleCancelEditFix} style={styles.button}>
                        <span className="material-icons">cancel</span>
                        Cancel
                      </button>
                    </div>
                  ) : (
                    <button onClick={handleAddFix} style={styles.button}>
                      <span className="material-icons">add</span>
                      Add Fix
                    </button>
                  )}
                ...
                <div style={styles.card}>
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', gap: '24px'}}>
                    <h2 style={styles.cardTitle}>Fixes</h2>
                    <div style={styles.searchContainer}>
                      <span className="material-icons" style={styles.searchIcon}>search</span>
                      <input
                        type="text"
                        placeholder="Search by Name, Coordinate, or Visibility Fix..."
                        value={fixSearchTerm}
                        onChange={(e) => setFixSearchTerm(e.target.value)}
                        style={styles.searchInput}
                      />
                    </div>
                  </div>
                    ...
                    <table style={styles.table}>
                      <thead style={styles.tableHeader}>
                        <tr>
                          ...
                          <th style={styles.tableCell}>Visibility Fix (1 or 2)</th>
                          <th style={styles.tableCell}>Visibility Fix (1 or 2)</th>
                          <th style={styles.tableCell}>Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {filteredFixes.map((fix) => (
                          <tr key={fix.id} style={styles.tableRow}>
                            <td style={styles.tableCell}>{fix.name}</td>
                            <td style={styles.tableCell}>{fix.coordinate}</td>
                            <td style={styles.tableCell}>{fix.visibilityFix1}</td>
                            <td style={styles.tableCell}>{fix.visibilityFix2}</td>
                            <td style={styles.tableCell}>
                              <div style={{display: 'flex', gap: '8px'}}>
                                <button
                                  onClick={() => handleEditFix(fix)}
                                  style={styles.editButton}
                                >
                                  <span className="material-icons" style={{fontSize: '16px'}}>edit</span>
                                  Edit
                        ...
                        {filteredFixes.length === 0 && (
                          <tr>
                            <td colSpan={5} style={styles.emptyState}>
                              No fixes found. Add your first fix above!
                            </td>
                          </tr>
                        )}
                      </tbody>
